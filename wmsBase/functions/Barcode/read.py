...
def read_t_relacion_codbarras(request, db_name):
    # Initialize query
    query = Q()
...
def read_t_relacion_codbarras_obj(request, db_name):
        '''
        ...
        for p in params: 
            if p not in fields: 
                raise ValueError("Field {} not found".format(p))
        ...
        try:    
            # Get the response
            response = TRelacionCodbarras.objects.using(db_name).filter(query,fechacrea__range=[start_date, end_date]).order_by('codbarrasasignado')[:default_limit]
            return response
