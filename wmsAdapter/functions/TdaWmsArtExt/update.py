...
def update_articles_extended(request, db_name, request_data=None):
    ...
    final_fields = {}
    for f in fields:
        if f != "tdawmsduk" and f != "tdawmsdpk":
            if f == "productoean":
    ...
    try:   
        product = read_articles_extended(request, db_name=db_name)
        if type(product) == str:
            return product
        else:
            if len(list(product)) == 1:
                ...
                return "No extended article found"
            else:
                return "Only one extended article can be updated at a time"
