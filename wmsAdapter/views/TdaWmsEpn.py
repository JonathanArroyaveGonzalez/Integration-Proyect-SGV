...
def epn(request):
    ...
    if request.method == 'GET':
        ...
        try:
            response = read_epn(request, db_name=db_name)
            ...
            print(e)
            return JsonResponse({'error': 'Error getting epn'}, safe=False, status=500)

  # Create a new article
    if request.method == 'POST':
        try:
            # response = 'created successfully's
            response = create_epn(request, db_name=db_name)
            if response == 'created successfully':
                return JsonResponse({'success': 'Epn created'}, safe=False, status=200)
            else:
                return JsonResponse({'error': response}, safe=False, status=500)
        except Exception as e:
            print(str(e))
            return JsonResponse({'error': 'Error creating epn'}, safe=False, status=500)
  ...
  # Update an article
    if request.method == 'PUT':
        ...
        try:
            response = update_epn(request, db_name=db_name)
 ...
 # Get storage by location
    if request.method == 'DELETE':
        ...
        try:
            response = delete_epn(request, db_name=db_name)
            ...
            print(e)
            return JsonResponse({'error': 'Error deleting epn'}, safe=False, status=500)
