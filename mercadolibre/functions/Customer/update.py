import logging
import os
import requests
import requests
from mercadolibre.functions.products.sync import get_wms_base_url
from mercadolibre.utils.api_client import (
    get_meli_api_base_url,
    make_authenticated_request,
)
from mercadolibre.utils.mappers import customer

logger = logging.getLogger(__name__)


def fetch_ml_user(user_id: str) -> dict | None:
    """Obtiene datos de usuario desde MercadoLibre API."""
    try:
        logger.info(f"Obteniendo datos ML para usuario {user_id}")
        base_url = get_meli_api_base_url()
        url = f"{base_url}users/{user_id}"
        response = make_authenticated_request("GET", url, timeout=30)
        if response.status_code == 200:
            return response.json()
        else:
            logger.error(f"Error ML {response.status_code}: {response.text}")
            return None
    except Exception as e:
        logger.error(f"Error obteniendo usuario {user_id} de ML: {str(e)}")
        return None


def get_wms_base_url() -> str:
    """Obtiene la URL base del WMS desde configuración o variable de entorno"""
    wms_url = os.getenv("WMS_BASE_URL", "http://localhost:8000")
    return wms_url.rstrip("/")


def update_customer_in_wms(ml_customer_id: str) -> dict:
    """
    Actualiza un cliente en WMS usando datos actuales de MercadoLibre.

    Args:
        ml_customer_id (str): ID del cliente en MercadoLibre

    Returns:
        dict: Respuesta en formato WMS {"created": [...], "errors": [...]}
    """
    try:
        # Validar entrada
        if not ml_customer_id or not ml_customer_id.strip():
            return {"created": [], "errors": ["ID de cliente requerido"]}

        # Obtener datos actuales de MercadoLibre
        ml_data = fetch_ml_user(ml_customer_id)
        if not ml_data:
            return {"created": [], "errors": ["Cliente no encontrado en MercadoLibre"]}

        # Mapear datos
        mapped_data = customer.CustomerMapper.from_meli_customer(ml_data)
        if not mapped_data:
            return {
                "created": [],
                "errors": ["Error transformando datos de MercadoLibre"],
            }

        # Configurar request
        headers = {
            "Content-Type": "application/json",
            "Accept": "application/json",
            "Authorization": os.getenv("WMS_API_TOKEN"),
        }

        wms_base_url = get_wms_base_url()
        wms_url = f"{wms_base_url}/wms/adapter/v2/clt"

        # ACTUALIZACIÓN: Usar PUT y enviar como array
        customer_payload = [mapped_data.to_dict()]

        response = requests.put(  # PUT en lugar de POST
            wms_url, json=customer_payload, headers=headers, timeout=30
        )

        response.raise_for_status()

        # Devolver respuesta directa del WMS
        wms_response = response.json()
        logger.info(f"Cliente ML {ml_customer_id} actualizado: {wms_response}")
        return wms_response

    except requests.HTTPError as e:
        logger.error(f"Error HTTP WMS: {e}")
        return {
            "created": [],
            "errors": [
                f"Error WMS HTTP {e.response.status_code if e.response else 'unknown'}"
            ],
        }
    except requests.RequestException as e:
        logger.error(f"Error de conexión WMS: {e}")
        return {"created": [], "errors": ["Error de conexión con WMS"]}
    except Exception as e:
        logger.error(f"Error inesperado actualizando cliente: {e}")
        return {"created": [], "errors": ["Error interno del sistema"]}
