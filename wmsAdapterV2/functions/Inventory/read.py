...
from wmsAdapterV2.models import TdaWmsArt, TdaWmsInv
...
def read_inventory(request, db_name):
    '''
    ...
    try:
        ...
        sort, params = get_sort(TdaWmsInv, params)
        fields = validate_fields(fields, TdaWmsInv)
        query, params = filter_by_field(TdaWmsInv, query, params)
    ...
    try:
        ...
        inventory, query, _ = exec_query_orm(json_query_orm)
...
def read_articles_obj(request, db_name):
    ...
    try:
        # Get the response
        response = TdaWmsArt.objects.using(db_name).filter(query, fecharegistro__range=[
            start_date, end_date]).order_by('id')[:default_limit]
        return response
