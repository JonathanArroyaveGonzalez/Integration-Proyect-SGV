...
class TdaWmsDuk(models.Model):
    ...
    pesopedido = models.DecimalField(db_column='pesoPedido', max_digits=17, decimal_places=4, blank=True, null=True)  # Field name made lowercase.
    pesoreservado = models.DecimalField(db_column='pesoReservado', max_digits=17, decimal_places=4, blank=True, null=True)  # Field name made lowercase.
    pesoenpicking = models.DecimalField(max_digits=17, decimal_places=4, blank=True, null=True)

    class Meta:
        managed = False
        db_table = 'TDA_WMS_DUK'
        unique_together = (('lineaidpicking', 'productoean', 'tipodocto', 'estadodetransferencia', 'doctoerp', 'numdocumento', 'unido'),)