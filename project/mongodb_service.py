...
class MongoDBService:
    ...
    def find_one(
        self, collection_name: str, filter_query: Dict = None, projection: Dict = None
    ) -> Optional[Dict]:
        """
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to find document in {collection_name}: {str(e)}"
            )
    ...
    def find(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to find documents in {collection_name}: {str(e)}"
            )
    ...
    def insert_one(self, collection_name: str, document: Dict) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to insert document in {collection_name}: {str(e)}"
            )
    ...
    def update_one(
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to update document in {collection_name}: {str(e)}"
            )

    def update_many(
        self, collection_name: str, filter_query: Dict, update_query: Dict
    ) -> int:
        """
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to update documents in {collection_name}: {str(e)}"
            )
    ...
    def delete_one(self, collection_name: str, filter_query: Dict) -> bool:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to delete document in {collection_name}: {str(e)}"
            )
                ...
                for doc in documents:
                    if "wms" in doc and "db" in doc["wms"]:
                        ...
                        if "db_base" in doc["wms"]:
                            db_connections[f"{collection_name}_base"] = doc["wms"][
                                "db_base"
                            ]
    ...
    def get_config(self, database: str, platform: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to get config for {database}/{platform}: {str(e)}"
            )
